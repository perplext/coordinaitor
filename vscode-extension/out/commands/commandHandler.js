...
class CommandHandler {
    constructor(client, taskProvider, agentProvider, knowledgeProvider, templateProvider) {
        this.client = client;
    ...
    async createTask(uri) {
        ...
        const panel = vscode.window.createWebviewPanel('createTask', 'Create Task', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async createKnowledgeEntry() {
        ...
        const panel = vscode.window.createWebviewPanel('createKnowledgeEntry', 'Create Knowledge Entry', vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
                ...
                case 'createEntry':
                    try {
                        const entry = await this.client.createKnowledgeEntry({
                            ...
                            type: message.type,
                            tags: message.tags ? message.tags.split(',').map((t) => t.trim()) : [],
                            metadata: {
    ...
    async autoCreateTaskFromFile(uri) {
    ...
    getTaskCreationWebviewContent(title, context, config) {
...
<body>
    ...
    getKnowledgeCreationWebviewContent(title, content, language) {
        return `
<!DOCTYPE html>
<html>
<head>
